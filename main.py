...
def _build_advice_system() -> str:
    ...
    return (
        ...
        "【各セクションの役割】\n"
        "1. voice_character：この声の一番の特徴・魅力を1〜2文で直接伝える。結論から始めること。『一方で』『しかし』などの転換は絶対に使わない。褒めるだけ。\n"
        "2. potential：声が整ったとき、相手にどう届くかを1〜2文で描く。希望・変化・未来のトーンで。課題・問題・欠点の言葉は使わない。最後は「実際の声をお聴きすれば、さらに正確な診断とあなただけのアドバイスをお伝えできます」で締める。\n"
        "3. next_step：使用しない\n\n"
        "【文体ルール】\n"
        "・小学生にも伝わる言葉で書く。専門用語・難しい表現は使わない\n"
        "・前置き・言い訳・遠回しな表現は一切なし。一文は短く、結論から\n"
        "・練習法・やり方・手順は書かない\n"
        "・各セクション2文以内。絶対に3文を超えない\n\n"
        "=== ボイストレーニング専門知識 ===\n\n"
        + knowledge
...
async def generate_advice(req: AdviceRequest):
    ...
    user_prompt = (
        ...
        f"セクション:\n"
        f"1. voice_character：強みを1〜2文で直接褒める。結論から書く。転換・但し書きは不要。\n"
        f"2. potential：声が整ったとき相手にどう届くかを1〜2文で。希望・変化のみ。最後は「実際の声をお聴きすれば、さらに正確な診断とあなただけのアドバイスをお伝えできます」で締める。\n"
        f"3. next_step：不要。空文字で返す。"
    )
