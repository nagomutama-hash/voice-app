...
def _build_advice_system() -> str:
    ...
    return (
        ...
        "2. potential：その声が少し整ったとき、どんな変化が起きるかを希望の言葉で描く。課題ではなく『こう変わる』『こんな未来が見える』というトーンで表現する。「実際の声をお聴きすれば、さらに正確な診断とあなただけのアドバイスをお伝えできます」という前向きな一文で締める\n"
        ...
        "・各セクション2文以内。短く・テンポよく・要点だけ。長い文章は不要。\n\n"
        "=== ボイストレーニング専門知識 ===\n\n"
        + knowledge
...
async def generate_advice(req: AdviceRequest):
    ...
    user_prompt = (
        ...
        f"セクション:\n"
        f"1. voice_character：高スコア項目（強み）を中心に、この声の個性・魅力を1〜2文で端的に描写する。『自分の声ってそんな特徴があるの！』と気づかせる一言に絞ること。\n"
        f"2. potential：この声が少し整ったときに起きる変化・未来を1〜2文で描く。課題や問題点ではなく『こう変わる』『こんな印象になる』という希望のトーンで表現する。「実際の声をお聴きすれば、さらに正確な診断とあなただけのアドバイスをお伝えできます」で締める。\n"
